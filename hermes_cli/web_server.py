...
async def get_sessions(
    ...
    activity across the compression chain) or ``created`` (by original
    start time). ``recent`` keeps a long-running conversation on the first
    page after it auto-compresses into a fresh continuation id, and floats a
    session back to the top as soon as it receives a new reply.
    """
